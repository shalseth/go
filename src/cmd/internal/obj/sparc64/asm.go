...
func aclass(ctxt *obj.Link, a *obj.Addr) int8 {
	...
	case obj.TYPE_MEM:
		...
		case obj.NAME_AUTO, obj.NAME_PARAM:
			return aclass(ctxt, autoeditaddr(ctxt, nil, a))
...
func span(ctxt *obj.Link, cursym *obj.LSym, newprog obj.ProgAlloc) {
	if cursym.Func().Text == nil || cursym.Func().Text.Link == nil { // handle external functions and ELF section symbols
		return
	}

	var pc int64 // relative to entry point
	for p := cursym.Func().Text.Link; p != nil; p = p.Link {
		o, err := oplook(autoeditprog(ctxt, cursym, p))
		if err != nil {
			ctxt.Diag("%s", err.Error())
		}
		p.Pc = pc
		pc += int64(o.size)
	}
...
func asmout(p *obj.Prog, o Opval, cursym *obj.LSym) (out []uint32, err error) {
	...
	if o.OpInfo == ClobberTMP {
		if usesTMP(&p.From) {
			return nil, fmt.Errorf("asmout: %q not allowed: synthetic instruction clobbers temporary registers", obj.Dconv(p, &p.From))
		}
		if isTMP(p.Reg) {
			return nil, fmt.Errorf("asmout: %q not allowed: synthetic instruction clobbers temporary registers", Rconv(int(p.Reg)))
		}
		if usesTMP(p.GetFrom3()) {
			return nil, fmt.Errorf("asmout: %q not allowed: synthetic instruction clobbers temporary registers", obj.Dconv(p, p.GetFrom3()))
		}
		if usesTMP(&p.To) {
	...
	// MOVRZ	R, $simm10, Rd
	case 48:
		*o1 = opcode(p.As) | rsr(p.From.Reg, p.GetFrom3().Offset, p.To.Reg) | 1<<13
	...
	case 50:
		...
		*o2 = opalu(AXOR) | rsr(p.To.Reg, 0, p.To.Reg)
		...
		rel.Sym = p.From.Sym
		rel.Add = p.From.Offset
		rel.Type = objabi.R_SPARC64_TLS_LE
...
// addrel appends an empty relocation to cursym and returns a pointer to
// it, so the encoder can fill the fields in place. Upstream removed
// obj.Addrel in favour of LSym.AddRel, which takes a Reloc by value.
//
// TODO(sparc64): LSym.AddRel also runs checkFIPSReloc for symbols in a
// FIPS section. Appending directly skips that check. Route through
// AddRel once the encoder is restructured to build a complete Reloc
// before appending it.
func addrel(cursym *obj.LSym) *obj.Reloc {
	cursym.R = append(cursym.R, obj.Reloc{})
	return &cursym.R[len(cursym.R)-1]
}
